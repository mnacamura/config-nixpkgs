...

{
  ...
  fishExtraConfig = let
    direnv = self.writeFishConfig "direnv" ''
      type -q direnv; and eval (direnv hook fish)
    '';
    git = self.writeFishConfig "git" ''
    ...
    '';
    less = self.writeFishConfig "less" ''
      if status is-login
    ...
    '';
    lsColors = with self; writeFishConfig "ls-colors" ''
      if status is-login
        eval (dircolors -c ${ls-colors}/share/LS_COLORS/LS_COLORS | sed 's|setenv|set -Ux|')
      end
    '';
    nix = with self; writeFishConfig "nix" (''
    ...
    '');
    misc = self.writeFishConfig "misc" ''
      if status is-interactive
        if type -q lp
  ...
  in super.buildEnv {
    ...
    paths = [ direnv git less lsColors nix misc ];
  };
