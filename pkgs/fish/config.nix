  ...
  configFiles = {
    ...
    less = writeFishConfig "less" ''
      if status is-login
        ...
        set -U LESS_TERMCAP_mb (printf "\e[5m")  # Begin blinking
        set -U LESS_TERMCAP_md (printf "\e[1m")  # Begin bold
        set -U LESS_TERMCAP_me (printf "\e[0m")  # End blinking/bold
        set -U LESS_TERMCAP_so (printf "\e[1;30;48;2;255;135;0m")  # Begin standout
        set -U LESS_TERMCAP_se (printf "\e[0m")  # End standout
        set -U LESS_TERMCAP_us (printf "\e[3;33m")  # Begin underline
        set -U LESS_TERMCAP_ue (printf "\e[0m")  # End underline
      end
    '';
